...
watch(url, () => {
  if (!url.value) return
  const currentType = types.value[state.type]?.toLowerCase() as 'flv' | 'hls'
  if (!currentType) return
  
  if (state[currentType]) {
    try {
      currentType == 'flv' ? state.flv?.switchURL(url.value, route.meta.site.id != 'bilibili') : state.hls?.loadSource(url.value)
    } catch (error) {
      console.error('Switch URL failed:', error)
      state.notice = '切换失败，请刷新重试'
      clearTimeout(noticeTimer)
      noticeTimer = setTimeout(() => state.notice = null, 3000)
    }
    return
  }
  
  if (currentType == 'flv' && Flv?.isSupported('video')) {
    try {
  ...
  if (currentType == 'hls' && Hls?.isSupported()) {
    try {
      ...
      state.notice = 'HLS播放器初始化失败'
    }
    return
  }
  
  state.notice = `不支持 ${currentType} 格式`;
  clearTimeout(noticeTimer)
  noticeTimer = setTimeout(() => state.notice = null, 3000)
})
...
const init = () => {
  ...
  useSiteFetch(site.id, 'getRoomDetail', { id }).then((data) => {
    ...
    state.notice = msg || '加载失败'
  }).finally(() => {
    clearTimeout(noticeTimer)
    if (state.notice && state.notice !== '未开播！') {
      noticeTimer = setTimeout(() => state.notice = null, 5000)
    }
  })
}
